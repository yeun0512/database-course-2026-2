...
```text
어떻게 테이블을 연결할지 등 구체적인 조건을 확정하지 않은 것이 많다. 
```

### Chapter 02에서 정리했던 한 행의 의미 중 수정할 부분이 있나요?

```text
exercise 부분을 보완해야할 것 같은데, 어떻게 할지 구체적인 수정 방향은 정하지 못하였다. 
```
---
...
```text
1. PostgreSQL 서버가 실행되는지 확인하는 방법: 시스템에 들어가서 실행되고 있는지 확인한다. 

2. DBeaver에서 PostgreSQL 연결을 만드는 방법: DBeaver 좌상단의 데이터베이스 연결 (플러그모양 아이콘) 을 활용한다. 

3. Host / Port / Database / Username의 의미: 호스트, 연결 방식, Postgres 등과 같이 실행하는 곳, 사용자 

4. ai_database_book에 연결되었는지 확인하는 방법: 직접 코드를 통해 확인해야 한다. 

5. 현재 위치를 확인하는 SQL: SELECT current_database();

6. 한 문장과 전체 스크립트 실행을 구분해야 하는 이유: 범위가 달라질 수 있는데, 데이터를 변경하는 내용이 실수로 같이 실행되는 경우 문제가 될 수 있다.

7. 비밀번호를 GitHub나 AI 프롬프트에 넣으면 안 되는 이유: 보안을 위해 필요하다. 
```
...
1. DBeaver와 PostgreSQL의 가장 중요한 차이는
   DBeaver는 DB에 전달하기 위한 것이고 DB는 Postgres로, 이는 실행을 담당한다는 점이다.

2. 내가 지금 어느 데이터베이스에 연결되어 있는지 확인할 때
   화면 이름만 보지 않고 SELECT current_database();로 확인해야 한다.

3. PostgreSQL 오류가 발생했을 때 가장 먼저 해야 할 일은
   오류 설명에서 어디에 오류가 있다고 지적하는지를 확인하는 것이다.

4. AI를 오류 해결에 사용할 때 가장 중요한 것은
   AI 가 제안한 내용을 적용하여 직접 다시 실행해보는 것이다.
```
...
- [0] `chapter03_answer.md`의 빈 필수 항목을 작성했다.
- [0] PostgreSQL과 DBeaver의 역할 차이를 설명했다.
- [0] `current_database/current_user/current_schema/search_path`를 실제로 확인했다.
- [0] `ai_database_book` 연결 여부를 SQL로 검증했다.
- [0] SQL 실행 범위 세 가지를 비교했다.
- [0] `setup_check.sql`을 실행했다.

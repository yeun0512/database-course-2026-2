...
| 본문의 핵심 | 나의 설명 |
| --- | --- |
| AI가 SQL을 만들어도 DB를 배워야 한다 | 네. AI 가 만든 SQL이 목적에 맞는 것인지 등을 확인해야 합니다. |
| 데이터 구조가 중요하다 | 네. 이해를 위해서 입니다. |
| SQL 결과를 검증해야 한다 | 네. 목적에 맞는지, 자료가 적절한지 검증해야 합니다. |
| 데이터 품질이 AI 답변 품질에 영향을 준다 | 네. AI가 부정확한 데이터를 활용하거나, 사실이 아닌 데이터를 활용할 수 있습니다. |
| 저장 방식은 데이터 양만 보고 선택하지 않는다 | 네. 구조나 방식도 중요합니다. |
